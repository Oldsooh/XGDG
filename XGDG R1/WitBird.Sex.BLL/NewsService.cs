...
namespace WitBird.Sex.BLL
{
    public class NewsService
    {
        ...
        public List<News> GetTopNews()
        {
            ...
            {
                conn.Open();
                result = DBRepository.SelectMore<News>("IsTop = 1 and IsActive = 1", "Id", false, conn, 10);
            }
            catch (Exception e)
            {
        ...
        public List<News> GetRandomNews()
        {
            ...
            {
                conn.Open();
                string sql = "select top 4  * from News where IsActive=1 order by newid()";
        ...
        public List<News> GetRelatedNews(int id)
        {
            ...
            try
            {
                ...
                {
                    string[] keys = news.Keywords.Split(',');
                    if (keys != null && keys.Count() > 0)
                    {
                        foreach (var key in keys)
                        {
                            string where = "Title like '%" + DBRepository.FilterSQL(key) + "%' or Keywords like '%" + DBRepository.FilterSQL(key) + "%'";
