    ...
    public class AlbumService
    {
        ...
        public AlbumDetail GetAlbumDetail(int id)
        {
            ...
            try
            {
                ...
                if (album != null)
                {
                    ...
                    {
                        sqlBuilder.Append("select top 5 * from Album where (");
                        char[] FilterChar = {' ', ',', '，' };
                    ...
                    sqlBuilder.Append("select top 5 * from Album where IsTop=1 and IsDelete=0 and IsShow = 1 order by newid();");
        ...
        public HomeData GetHomeData()
        {
            ...
            try
            {
                ...
                //New upload
                sqlBuilder.Append("select top 5 * from Album where IsDelete=0 and IsShow=1 order by id desc; ");
                //Recomended
...
//                    and (convert(date,UpdatedTime) between convert(date,dateadd(dd,-7,getdate())) and convert(date,getdate())) 
                //                    order by ViewTime desc;");
                //Sexy
                sqlBuilder.Append("select top 6 * from Album where CategoryId = 'sexy' and IsDelete=0 and IsShow = 1 order by id desc;");
                //Pure
                sqlBuilder.Append("select top 6 * from Album where CategoryId = 'pure' and IsDelete=0 and IsShow = 1 order by id desc;");
                //Star
                sqlBuilder.Append("select top 6 * from Album where CategoryId = 'star' and IsDelete=0 and IsShow = 1 order by id desc;");
                //Costume
